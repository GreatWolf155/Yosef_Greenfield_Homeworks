DROP TABLE IF EXISTS eurovision_winners;
...
-- 4
SELECT song_length_seconds
FROM song_details
ORDER BY song_length_seconds ASC
LIMIT 1;

-- 5
SELECT *
FROM eurovision_winners e
JOIN song_details s ON s.year = e.year;
...
-- 11
SELECT s.song_length_seconds, e.song_name
FROM song_details s
JOIN eurovision_winners e ON e.year = s.year
...
-- 15
SELECT MIN(year) AS first_time, MAX(year) AS last_time
FROM eurovision_winners
WHERE country = 'Israel';

-- 16
SELECT e.song_name, e.country, s.year, s.song_length_seconds
FROM song_details s
...
FROM eurovision_winners e
JOIN song_details s ON s.year = e.year
WHERE s.song_length_seconds > (SELECT AVG(song_length_seconds) FROM song_details);
...
-- 19
SELECT e.country, AVG(s.song_length_seconds) AS avg_song_length
FROM eurovision_winners e
JOIN song_details s ON s.year = e.year
GROUP BY e.country;
