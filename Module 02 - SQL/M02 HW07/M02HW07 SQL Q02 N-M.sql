...
-- 5
-- Write a query to show each sold product with order_no, address, phone, product_name, qty, unit_price, and line total (qty * unit_price)
SELECT
	o.order_no,
	o.address,
	o.phone,
...
-- 6
-- Write a query to list each order with total item count (SUM(qty)) and total price (SUM(qty * unit_price))
SELECT
	o.order_no,
...
-- 7
-- Write a query to list each order with all product names. Make sure to print all products of the same order before going to the next order
SELECT
	o.order_no,
	p.name AS product_name
FROM orders o
...
-- 8
-- Write a query to calculate each phone (or address) and the sum of all orders
SELECT
	o.phone,
...
-- 9
-- Write a query to show orders that have no products in sales
SELECT
	o.order_no,
	o.address
FROM orders o
